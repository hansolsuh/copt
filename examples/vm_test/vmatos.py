...
if __name__ == "__main__":
    ...
    if flag1 == '1':
        ...
        for i, lbd in enumerate(lbd):
            ...
            ax.set_title("Tomography " r"$\lambda=%s$" % lbd)
    ...
    if flag2 == '1':
        ...
        for i, beta in enumerate(all_betas):
            ...
            ax[i].set_title("Overlapping Lasso " r"$\lambda=%s$" % beta)
    ...
    if flag3 == '1':
        ...
        ax.plot(sample_MM_obj[0], label='VM-MM')
    ...
    if flag4 == '1':
        ...
        #1. VM-BB
        cb_adatos_vm = cp.utils.Trace()
        adatos_avm   = cp.minimize_three_split(f_copt.f_grad,np.zeros(n_features_port), G2.prox,G1.prox,step_size=port_stepsize,max_iter=max_iter, tol=1.e-6,line_search=False,
                                               vm_type=1,Hinv=Hinv,callback=cb_adatos_vm)
        ...
        adatos_atos = cp.minimize_three_split(f_copt.f_grad,np.zeros(n_features_port), G2.prox,G1.prox,step_size=port_stepsize, max_iter=max_iter, tol=1.e-6,callback=cb_atos)
        trace_obj = None
        ...
        G2 = portfoliodata.f3_copt(port_r,port_m)
        cb_tos = cp.utils.Trace()
        adatos_tos = cp.minimize_three_split(f_copt.f_grad,np.zeros(n_features_port), G2.prox,G1.prox,step_size=port_stepsize, max_iter=max_iter, tol=1.e-6,callback=cb_tos,line_search=False)
        trace_obj = None
